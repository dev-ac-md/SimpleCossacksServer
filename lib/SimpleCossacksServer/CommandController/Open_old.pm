...
my @PUBLIC = qw[
  ...
  tournaments gg_cup_thanks_dgl get_players_list
];
...
sub try_enter {
  ...
  if($p->{RESET}) {
    $self->logout_player($h->server, $h->connection->data->{account});
    ...
    if (ref $result eq 'HASH' && $result->{playerDetails} && ref $result->{playerDetails} eq 'ARRAY') {
        ...
        for my $player (@sorted_players) {
            if (ref $player eq 'HASH' && exists $player->{nickName}) {
                ...
                my $playTimeStr = $playTimeSeconds < 3600 ? int($playTimeSeconds / 60) . 'm' : int($playTimeSeconds / 3600) . 'h';
